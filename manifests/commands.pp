...
# the winpe image
  ...
  define petools::commands::ps_get_drivers_from_web(
    $url,
    $file,
  ){
    exec { "${name}":
...
#
  define petools::commands::ps_get_msi_from_web (
    $url,
    $file,
  ) {
    exec { $name:
...
# uses 7zip to extract an archive
  ...
  define petools::commands::extract_archive ($archivefile, $archivepath) {
